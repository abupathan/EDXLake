 ...
 * - Paginates retention schedule table (accessible controls)
 */
...
function buildTOC(){
  const toc = $('#toc-list');
  toc.innerHTML = '';
  const headings = $$('#retention-main h2');
...
/* ---------- Retention schedule pagination ---------- */
function paginateSchedule(){
  const tbody = $('#retention-table tbody');
  const rows  = Array.from(tbody.querySelectorAll('tr'));
  const sizeSel = $('#sched-size');
  const pageInput = $('#sched-page');
  const pagesLabel = $('#sched-pages-label');
  ...
  const render = () => {
    ...
    const host = document.querySelector('[data-paginate-sched]');
