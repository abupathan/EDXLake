 ...
 * - Paginates DSA table (accessible controls)
 */
...
function buildTOC(){
  const toc = $('#toc-list');
  toc.innerHTML = '';
  const headings = $$('#sharing-main h2');
...
/* ---------- DSA table pagination ---------- */
function paginateDSA(){
  const tbody = $('#dsa-table tbody');
  const rows  = Array.from(tbody.querySelectorAll('tr'));
  const sizeSel = $('#dsa-size');
  const pageInput = $('#dsa-page');
  const pagesLabel = $('#dsa-pages-label');
  const count = $('#dsa-count');
  ...
  const render = () => {
    ...
    const host = document.querySelector('[data-paginate-dsa]');
